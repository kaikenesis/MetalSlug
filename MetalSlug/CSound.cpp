...
{
	if (nullptr == GetSoundManager()->GetSoundDevice())
		return S_FALSE;
...
bool CSound::LoadWaveSound(const wstring& _strPath)
{
	...
	if (FAILED(GetSoundManager()->GetSoundDevice()->CreateSoundBuffer(&m_tBuffInfo, &m_pSoundBuffer, NULL)))
	{
...
{
	GetSoundManager()->RegisterToBGM(this);
